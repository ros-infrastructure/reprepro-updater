...
if not [d for d in options.distro if d in ALL_DISTROS]:
    parser.error("invalid distros %s, not in %s" %
                 (options.distro, ALL_DISTROS))
...
dist = conf.DistributionsFile(ALL_DISTROS, ALL_ARCHES,
                              options.key, updates_generator)
...
if not options.no_cleanup:
    for distro in options.distro:
        run_cleanup(repo_dir, options.rosdistro, distro,
                    options.arch, options.commit)
for distro in options.distro:
    run_update(repo_dir, dist, updates_generator,
